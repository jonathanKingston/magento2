...
define([
        ...
        for (i = 0; i < dataElements.length; i++) {
            if (dataElements[i] && dataElements[i].id) {
                ...
                if (!useCache || !varienWindowOnloadCache[dataElements[i].id]) {
                    jQuery(dataElements[i]).on('change', dataElements[i].setHasChanges.bind(dataElements[i]));
    ...
    jQuery(window).load(varienWindowOnload);
    ...
    RegionUpdater.prototype = {
        ...
        initialize: function (
            ...
            jQuery(this.countryEl).on('change', this.update.bind(this));
        },
